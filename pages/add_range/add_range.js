  ...
  updateRange: function(e) {
    ...
    if (self.data.images.length > 0) {
      ...
      self.doUploadImg(self.data.images, function(files) {
        self.doUpdateRange(files, type)
      })
    } else {
      self.doUpdateRange([], type)
    }
  },
  doUpdateRange: function(files, type) {
    // 添加请求
    var self = this
    var tips = ''
    if (!self.data.title || !self.data.startTime) {
      ...
      app.request({
        showLogin: true,
        url: '/api/time_whisper/update_range',
        method: 'POST',
        data: {
          id: self.data.rangId || '', // 根据是否有id 进行更新或是添加操作
