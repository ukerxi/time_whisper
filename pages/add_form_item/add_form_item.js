...
Page({
  ...
  doUpdateInfo: function(files, type) {
    ...
    if (!self.data.title || !self.data.startTime) {
      ...
      app.request({
        showLogin: true,
        url: '/api/time_whisper/update_form_item',
        method: 'POST',
        data: {
          ...
          num: parseFloat(self.data.num) || 0,
          listType: self.data.listType,
          relateId: self.data.relateId,
  ...
  getInfo: function(id) {
    ...
    app.request({
      showLogin: true,
      url: '/api/time_whisper/get_form_detail',
      method: 'POST',
      data: {
        id: id
      },
